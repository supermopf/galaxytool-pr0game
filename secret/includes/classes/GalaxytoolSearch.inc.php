...
*/
		...
		if ($this->selection['filter_nap']) {
			...
			array_push($wheres, "(a.diplomatic_status!='nap' OR p.alliance_id = 0)");
		}
		if ($this->selection['filter_allied']) {
			array_push($wheres, "p.diplomatic_status!='ally'");
			array_push($wheres, "(a.diplomatic_status!='ally' OR p.alliance_id = 0)");
		}
		if ($this->selection['filter_own']) {
			array_push($wheres, "p.diplomatic_status!='own'");
			array_push($wheres, "(a.diplomatic_status!='own' OR p.alliance_id = 0)");
		}
		if ($this->selection['filter_wing']) {
			array_push($wheres, "p.diplomatic_status!='wing'");
			array_push($wheres, "(a.diplomatic_status!='wing' OR p.alliance_id = 0)");
		}
