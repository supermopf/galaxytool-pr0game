...
class ReportParser extends XMLParserGlobal
{
    ...
    private function get_report_data($report_DOMNode)
    {
        ...
        $return_value = [];
        if ($report_DOMNode->hasAttribute("playername")) {
        ...
        switch ($return_value["scandepth"]) {
            case "fleet":
                $return_value["scandepth"] = 2;
                break;
            case "defence":
                $return_value["scandepth"] = 3;
                break;
            case "buildings":
                $return_value["scandepth"] = 4;
                break;
            case "research":
                $return_value["scandepth"] = 5;
                break;
            default:
                $return_value["scandepth"] = 1;
                break;
        }
    ...
    private function delete_existing_reports_from_array(array &$reports)
    {
        ...
        //check archive too
        $query = "SELECT `msg_id` FROM `report_archive` WHERE `msg_id` IN ('" . implode("','", $report_ids) . "')";
        $stmt = $this->query($query);
        if (!$stmt) {
            $this->error_object = new ErrorObject(ErrorObject::severity_error, "DB error occurred while checking for existing reports");
            $this->error_object->add_child_message($this->get_db_error_object());
    ...
    private function update_reports_at_database(array $reports, $userid)
    {
        ...
        foreach ($reports as $report) {
            ...
            if (isset($entries_array[DB_REPORT_ARRAY[B_ROCKETSILO]["DBFIELD"]]) && $entries_array[DB_REPORT_ARRAY[B_ROCKETSILO]["DBFIELD"]] > 3 && $entries_array[DB_REPORT_ARRAY[RS_IMPULSEENGINE]["DBFIELD"]] > 0) { // rocket silo and impulse tech
