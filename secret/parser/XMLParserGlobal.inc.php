...
class XMLParserGlobal extends DBHandler {
	...
	protected function get_players_data($xml_DOMNode) {
		...
		if($return_value["playerid"] == 0 && $return_value["playername"] != ""){
			$query = "SELECT `id`,`playername`,`ogame_playerid` FROM `players` WHERE (`playername` = ".DB::getDB()->quote($return_value["playername"]).")";
			$stmt = $this->query($query);
			if (!$stmt) {
				$this->error_object = new ErrorObject(ErrorObject::severity_error , "DB error occurred while getting playerid for player in XMLGlobalParser");
				$this->error_object->add_child_message($this->get_db_error_object());
				return false;
			}
			
			while ($line = $stmt->fetch(PDO::FETCH_OBJ)) {
				$return_value["playerid"] = $line->ogame_playerid;
			}
		}
